...
app.delete("/api/users/:name/courses/:courseName", (req, res) => {
  const { name } = req.params;
  const user = users.find(userElem => userElem.name === name);
  ...
  if (courseIndex !== -1) user.enrolledCourses.splice(courseIndex, 1);
...
app.get("/api/courses/:name", (req, res) => {
  const course = courses.find(
    courseElem => courseElem.name === req.params.name
  );
...
app.post("/api/users", (req, res) => {
  ...
  if (isRegistered) return res.status(403).send("Already Registered");
